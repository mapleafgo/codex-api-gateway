...
- **观测台**：7 张指标卡（上游调用量 / 输入 / 输出 / 缓存创建 / 缓存命中 / 缓存 Token 命中率 / 指标丢弃量）+ `供应商 × 模型` 用量聚合表 + 最近 1000 条请求历史（按模型/供应商/Code 过滤，耗时渐变色条）。指标按上游尝试聚合；总输入已按协议归一化，Anthropic 为 `input + cache_read + cache_creation`，Chat/Responses 使用已含缓存 Token 的 `input`；命中率为缓存读取 Token / 归一化总输入 Token。
...
- **性能隔离**：指标采集走独立 goroutine + 带缓冲 channel，请求路径只做一次非阻塞投递，channel 满即丢弃并累计到「指标丢弃量」，绝不拖慢转发；管理端异常不影响 `/v1/*`。指标为进程生命周期内的内存累计值，重启后清零。
