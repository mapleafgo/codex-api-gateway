...
> 注意：本文保留 2026-07-16 时的首批行为描述，其中部分条目（如 structured output 合成工具、`local_shell` 选择语义、`apply_patch` 声明形态）已被后续版本移除或改写。实际状态一律以主矩阵 `docs/protocol-coverage.md` 与当前代码为准。
