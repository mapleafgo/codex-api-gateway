...
- 网关按 anthropic 源 `options.cache_enabled` 自主控制 Anthropic `cache_control`（TTL 固定 5m，插件 schema 缺省 true），不依赖 OpenAI prompt cache key。MCP 由 Codex 客户端本地执行：网关只声明扁平 `mcp__<server>__<tool>` function，不再注入 beta MCP 配置。
...
- **单笔源请求总时长超时（`breaker.request_timeout`）**：新增全局默认 120s +
  可选 per-source 覆盖；到点即终止该笔上游调用。未出内容时按既有失败逻辑计熔断
  并允许 failover；已出内容时源锁定、客户端保留已收内容并以 `response.failed`
  收尾（r 透传不代补上游事件，网关主动中止场景由 server 兜底补发终态）。服务端
  超时在终态/日志/指标中以 failed/504/reason=timeout 标记，与客户端取消
  （canceled/499）可区分。与首字节超时职责不同：首字节只约束"等待首个事件"并
  触发该源失败切换；单笔总时长约束"该笔请求全程"，两个机制按先到先生效，总时长
  不被首个事件停止。
...
| `max_output_tokens` | `max_tokens` | `lossy_supported` | 客户端值优先；未设置时使用 anthropic 源 `options.default_max_tokens`，插件 schema 缺省 16384。**注意**：OpenAI `max_output_tokens` 预算含 reasoning token，Anthropic `max_tokens` 仅限制可见输出（thinking 由 `budget_tokens` 独立控制），两者计数口径不一致 |
