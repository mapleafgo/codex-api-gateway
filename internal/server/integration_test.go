...
func TestIntegrationToolMultiRound(t *testing.T) {
	toolUseLines := []string{
		`{"type":"message_start","message":{"id":"m_tool1","model":"claude"}}`,
		`{"type":"content_block_start","index":0,"content_block":{"type":"tool_use","id":"call_xyz","name":"get_weather"}}`,
	...
	round2Lines := []string{
		`{"type":"message_start","message":{"id":"m_tool2","model":"claude"}}`,
		`{"type":"content_block_start","index":0,"content_block":{"type":"text"}}`,
		`{"type":"content_block_delta","index":0,"delta":{"type":"text_delta","text":"Sunny"}}`,
		`{"type":"content_block_stop","index":0}`,
		`{"type":"message_delta","delta":{"type":"message_delta","stop_reason":"end_turn"},"usage":{"input_tokens":8,"output_tokens":2}}`,
		`{"type":"message_stop"}`,
	}

	upstream, _ := mockBackend([]mockResp{
		{lines: toolUseLines},
		{lines: round2Lines},
	})
	defer upstream.Close()
	...
	if doneEv.data["arguments"] != `{"city":"SF"}` {
		t.Fatalf("expected arguments={\"city\":\"SF\"}, got %v", doneEv.data["arguments"])
	}

	// Find the function_call output item to get the call_id for round 2.
	var callID string
	for _, e := range events {
	...
	if callID == "" {
		t.Fatalf("could not find function_call call_id in events")
	}
	if callID != "call_xyz" {
		t.Fatalf("expected call_id=call_xyz, got %s", callID)
	}

	// Extract the response ID from round 1 for previous_response_id.
	completed := findEvent(t, events, "response.completed")
	respID := completed.data["response"].(map[string]any)["id"].(string)

	// Round 2: POST with function_call_output + previous_response_id.
	round2Body := fmt.Sprintf(`{"model":"gpt-5","previous_response_id":%q,"input":[{"type":"function_call_output","call_id":%q,"output":"Sunny, 72F"}],"stream":true}`,
		respID, callID)
	events2 := postResponses(t, ts, round2Body)

	// Round 2 should succeed with text output.
	requireEvent(t, events2, "response.output_text.delta")
	requireEvent(t, events2, "response.completed")

	// Verify the session was enriched: the upstream should have received the
	// tool_use + tool_result in its messages. We can verify this indirectly:
	// the round 2 response should carry the previous_response_id echo.
	completed2 := findEvent(t, events2, "response.completed")
...
func TestIntegrationPostTerminalTransportFailurePersistsVisibleOutput(t *testing.T) {
	var calls atomic.Int64
	var requestBodies []string
	var requestMu sync.Mutex
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		f.Flush()
		for _, line := range []string{
			`{"type":"message_start","message":{"id":"m_post_terminal_reset","model":"claude"}}`,
			`{"type":"content_block_start","index":0,"content_block":{"type":"text"}}`,
			`{"type":"content_block_delta","index":0,"delta":{"type":"text_delta","text":"visible output"}}`,
			`{"type":"content_block_stop","index":0}`,
			`{"type":"message_delta","delta":{"type":"message_delta","stop_reason":"end_turn"},"usage":{"input_tokens":5,"output_tokens":2}}`,
			`{"type":"message_stop"}`,
		} {
			if _, err := io.WriteString(w, "data: "+line+"\n\n"); err != nil {
				t.Fatalf("write upstream response: %v", err)
	...
	if completedCount != 1 {
		t.Fatalf("expected exactly one response.completed, got %d: %v", completedCount, eventTypes(first))
	}
	requireNotEvent(t, first, "response.failed")
	if _, ok := srv.sess.Get("m_post_terminal_reset"); !ok {
		t.Fatal("completed response must be persisted for replay after post-terminal transport failure")
	}

	postResponses(t, ts, `{"model":"gpt-5","previous_response_id":"m_post_terminal_reset","input":"follow-up","stream":true}`)
	requestMu.Lock()
	secondRequest := requestBodies[1]
	requestMu.Unlock()
	if !strings.Contains(secondRequest, "visible output") {
		t.Fatalf("previous_response_id replay omitted visible output: %s", secondRequest)
	}
}
...
func TestIntegrationReasoningPlaintext(t *testing.T) {
	...
	// Verify session store has the signature for enrichment round-trip.
	respID := resp["id"].(string)
	entry, ok := srv.sess.Get(respID)
	if !ok {
		t.Fatalf("session store missing entry for %s", respID)
	}
	var storedSig bool
	for _, item := range entry.Items {
		if item.Type == "reasoning" && item.Signature == "Sig12345" {
			storedSig = true
		}
	}
	if !storedSig {
		t.Fatalf("session store missing signature for round-trip: %+v", entry.Items)
	}
}
...
func TestIntegrationRedactedThinking(t *testing.T) {
	...
	// Session store should have the encrypted content for round-trip.
	respID := resp["id"].(string)
	entry, ok := srv.sess.Get(respID)
	if !ok {
		t.Fatalf("session store missing entry")
	}
	var storedEnc bool
	for _, item := range entry.Items {
		if item.Type == "reasoning" && item.EncryptedContent == "ENCRYPTED_BLOB_123" {
			storedEnc = true
		}
	}
	if !storedEnc {
		t.Fatalf("session store missing encrypted_content: %+v", entry.Items)
	}
}
...
func TestIntegrationFailoverSavesActualSourceName(t *testing.T) {
	...
	requireEvent(t, events, "response.completed")
	entry, ok := srv.sess.Get("m_actual_source")
	if !ok {
		t.Fatalf("session entry for completed response was not saved")
	}
	if entry.SourceName != "B" {
		t.Fatalf("session source = %q, want actual successful source B", entry.SourceName)
	}
}
